...
var dayfivenaughtystrings = [4]string{"ab", "cd", "pq", "xy"}
var vowels = [5]byte{'a', 'e', 'i', 'o', 'u'}

func isNaughty(s string) bool {
	for i := 0; i < len(s)-1; i++ {
		substr := s[i : i+2]
		for _, naughtystring := range dayfivenaughtystrings {
			if substr == naughtystring {
				return true
			}
		}
	}
...
func isNice(s string) bool {
	...
	for i := 0; i < len(s); i++ {
		...
		if !hasDouble && !(i == len(s)-1) { // index bounding
			nextch := s[i+1]
			if nextch == ch {
				hasDouble = true
			}
