...
type XmasLight struct {
	On bool
}

func (x *XmasLight) TurnOn() {
	x.On = true
}

func (x *XmasLight) TurnOff() {
	x.On = false
}

func (x *XmasLight) Toggle() {
	x.On = !x.On
}

type NewXmasLight struct {
	Brightness int
}

func (x *NewXmasLight) TurnOn() {
	// increase brightness by one
	x.Brightness++
}

func (x *NewXmasLight) TurnOff() {
	// decrease brightness by one, min zero
	if x.Brightness > 0 {
		x.Brightness--
	}
}

func (x *NewXmasLight) Toggle() {
	// increase brightness by two
	x.Brightness++
	x.Brightness++
}

func doInstruction(s string, lights [][]Light) {
	instructions := strings.Fields(s)
	var action string
	switch instructions[0] {
	case "toggle":
		action = "Toggle"
		instructions = instructions[1:] // strip off "toggle"
	case "turn":
		...
		instructions = instructions[2:] // strip off "turn" and "off"
	}
	start_pos := strings.Split(instructions[0], ",")
	start_x, _ := strconv.Atoi(start_pos[0])
	start_y, _ := strconv.Atoi(start_pos[1])
	instructions = instructions[2:] // strip off "(x,y)" and "through"
	end_pos := strings.Split(instructions[0], ",")
	end_x, _ := strconv.Atoi(end_pos[0])
	end_y, _ := strconv.Atoi(end_pos[1])

	for y := start_y; y <= end_y; y++ {
		for x := start_x; x <= end_x; x++ {
			reflect.ValueOf(lights[y][x]).MethodByName(action).Call(nil)
			// lights[y][x].action()
		}
	}
}

func DaySix(p *puzzle.Puzzle) {
	lights := make([][]Light, gridSize)
	for y := range lights {
		lights[y] = make([]Light, gridSize)
		for x := 0; x < gridSize; x++ {
			lights[y][x] = &XmasLight{}
		}
	}

	newLights := make([][]Light, gridSize)
	for y := range newLights {
		newLights[y] = make([]Light, gridSize)
		for x := range newLights[y] {
			newLights[y][x] = &NewXmasLight{}
		}
	}

	for _, instruction := range strings.Split(daysixinput, "\n") {
		doInstruction(instruction, lights)
		doInstruction(instruction, newLights)
	}

	lightsOn := 0
	for _, row := range lights {
		for _, light := range row {
			if light.(*XmasLight).On {
				lightsOn++
			}
		}
	}
	...
	brightness := 0
	for _, row := range newLights {
		for _, light := range row {
			brightness += light.(*NewXmasLight).Brightness
		}
	}

	p.AddSolution(strconv.Itoa(lightsOn))
	p.AddSolution(strconv.Itoa(brightness))
}

const gridSize = 1000
const daysixinput = `toggle 461,550 through 564,900
