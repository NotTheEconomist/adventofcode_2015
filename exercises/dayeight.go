...
func DayEight(p *puzzle.Puzzle) {
	reCodeChars, codeChars, stringChars := 0, 0, 0
	for _, line := range strings.Split(dayeightinput, "\n") {
		codeChars += len(line)
		reCodeChars += len(strconv.Quote(line))
		if unquoted, err := strconv.Unquote(line); err != nil {
			panic("Bad line! " + line)
		} else {
			stringChars += len(unquoted)
		}
	}
	p.AddSolution(strconv.Itoa(codeChars - stringChars))
	p.AddSolution(strconv.Itoa(reCodeChars - codeChars))
}

const dayeightinput = `"qxfcsmh"
